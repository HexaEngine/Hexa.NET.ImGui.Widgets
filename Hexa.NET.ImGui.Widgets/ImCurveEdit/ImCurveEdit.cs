namespace Hexa.NET.ImGui.Widgets.ImCurveEdit
{
    ...
    public static unsafe class ImCurveEdit
    {
        ...
        {
            x = ImMath.Clamp((x - edge0) / (edge1 - edge0), 0.0f, 1.0f);
            return x * x * (3 - 2 * x);
        }
        ...
        private static int DrawPoint(CurveContext ctx, Vector2 pos, bool edited)
        {
            ...
            draw->AddConvexPolyFilled(offsets, 4, 0xFF000000);
        ...
        static UnsafeList<Vector2> originalPoints;

        public static int Edit(CurveContext ctx, Vector2 size, uint id, ImVector<EditPoint>* selectedPoints = null)
        {
            int ret = 0;
            var io = ImGui.GetIO();
            ...
            if (hovered)
            {
                if (MathF.Abs(io.MouseWheel) > ImMath.FloatEpsilon)
                {
                    float r = (io.MousePos.Y - pos.Y) / size.Y;
                    float ratioY = ImMath.Lerp(ctx.Min.Y, ctx.Max.Y, r);
            ...
            for (int p = 0; p < ptCount - 1; p++)
            {
                ...
                if (curveType == CurveType.CurveSmooth || curveType == CurveType.CurveLinear)
                {
                    ...
                    for (nuint substep = 0; substep < subStepCount - 1; substep++)
                    {
                        ...
                        Vector2 pos1 = ctx.PointToScreen(new(sp1.X, ImMath.Lerp(p1.Y, p2.Y, rt1)));
                        Vector2 pos2 = ctx.PointToScreen(new(sp2.X, ImMath.Lerp(p1.Y, p2.Y, rt2)));
